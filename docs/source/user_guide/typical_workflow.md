...
First, we fetch animal data via the colony management system's api, and convert it to OSCaR's [standard table format](). At the moment, only [PyRAT](https://www.scionics.com/pyrat.html) is supported - but we hope to expand to more systems in future.
...
- Have length matching the `n_mutation` in your standard table for the line of interest. So e.g. if `n_mutations=1` for this line, your tuples may look like: `(Genotype.WT,)`, or if `n_mutations=3` it may be: `(Genotype.HET, Genotype.HOM, Genotype.WT)`. All the tuples you provide should have the same length.
